...
#include "_HMod/Frames_args.h"
#include "_HMod/x264HMod.h"

#include "_HMod/HMEC/CHIP/Misc/Precalcs.h"
...
static int encode(x264_param_t* param, cli_opt_t* opt, Frames_args frames_args);
...
int main( int argc, char **argv )
{
	...
	x264HMod_init(argc, argv, &frames_args);
    ...
    if( !ret )
		...
		ret = encode(&param, &opt, frames_args);


	// schlto 01.04.2016
	if(frames_args.enable_HMod)
		precalcs_free();
...
static int encode_frame(x264_t* h, hnd_t hout, x264_picture_t* pic, int64_t* last_dts,
 x264_param_t param, int i_frame, Frames_args frames_args)
...

{
	...
	// schlto 01.04.2016
	if(frames_args.enable_HMod)
		x264HMod(pic, param, i_frame, frames_args);
...
#   define  MAX_PTS_WARNING 3 /* arbitrary */
    ...
    for( ; !b_ctrl_c && (i_frame < param->i_frame_total || !param->i_frame_total); i_frame++ )
    {
		...
		i_frame_size = encode_frame(h, opt->hout, &pic, &last_dts, *param, i_frame, frames_args);
    ...
    while( !b_ctrl_c && x264_encoder_delayed_frames( h ) )
    {
		...
		i_frame_size = encode_frame(h, opt->hout, NULL, &last_dts, *param, i_frame, frames_args);
