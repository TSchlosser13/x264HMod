...
help:
	@echo "/*****************************************************************************"
	@echo " * help : x264HMod - v1.0 - 01.04.2016"
	@echo " *****************************************************************************"
...
_update:
	...
	@if [ -d _HMod/HMEC ]; then \
		...
		cd _HMod; \
		git clone https://github.com/TSchlosser13/HMEC HMEC; \
		cd ..; \
	fi
	@if [ -d _HMod/HMEC/CHIP ]; then \
		cd _HMod/HMEC/CHIP;    \
		echo "CHIP: git pull"; \
		git pull;              \
		cd ../../..;           \
...
SRCS_CHIP = $(p)/Misc/pArray2d.c $(p)/Misc/Precalcs.c \
            $(p)/CHIP/Hexint.c $(p)/CHIP/Hexarray.c $(p)/CHIP/Hexsamp.c \
            $(p)/HDCT/HDCT.c $(p)/HFBs/HFBs.c \
            	$(p)/Quant/coefTables.c $(p)/Quant/Quant.c

SRCS_x264HMod = $(SRCS_CHIP) _HMod/HMEC/HMEC/HMEC.c _HMod/x264HMod.c
